# TRUTH.md — Kristina-Irwin-Site Profile
...
- **Name:** Kristina-Irwin-Site
- **Slug:** kristina-irwin-site
- **Type:** Self-contained Hermes AI profile
- **OS/Shell:** Windows 10/11 + PowerShell
- **Purpose:** Kristina Irwin one-page website — client SiteGround rebuild (successor to archived Kristina-Irwin).
...
- **Memory Store:** Mem0 — collection `kristina-irwin-site_memories`
- **Telegram Gateway:** Hermes profile gateway for iPhone two-way chat

## Core Rules

- **Voice:** Edge Liam (`en-CA-LiamNeural`) primary for ritual speaks (Start/End Project, explicit `draven:speak`). OmniVoice optional restore / Edge-failure fallback. Ritual-only — never auto-read chat replies or Mem0 recall. See `VOICE-WORKFLOW.md` → Restore OmniVoice.
...
- Keep memories in the `kristina-irwin-site_memories` collection only.
