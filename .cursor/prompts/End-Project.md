# End Project — Kristina-Irwin-Site Day-End Closeout
...
Optional hub logger (Kristina-Irwin-Site):

```powershell
npm run log:session -- "[short title]: [summary of session]"
```

Update **`HISTORY.md`** (Kristina-Irwin-Site) when milestone-level.
...
Update `H:\Vader_Vault\01_Projects/Kristina-Irwin-Site.md` hub if decisions/learnings changed. See `.cursor/rules/vader-vault.mdc`.
...
```powershell
npm run draven:speak -- "Great work Jon. Kristina-Irwin-Site session saved. Until next time."
```
...
```
Great work Jon — Kristina-Irwin-Site session saved.
