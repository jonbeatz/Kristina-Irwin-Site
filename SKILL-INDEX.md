...
- **Placeholder-aware:** re-substitutes each project's values (`{PROFILE_NAME}`, `{PROFILE_ENV_PREFIX}`, `{WORKSPACE}`, `{GITHUB_REPO}` from git remote, etc.). Non-derivable values live in `.cursor/docs/.docsync.json` (e.g. `PROFILE_DESCRIPTION`).
